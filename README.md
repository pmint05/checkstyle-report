...
+ `Checker.process`: implements `RootModule.process`: filters files by extention, delegates to `FileSetCheck.process` to process each file, collects and returns violations
+ `AbstractFileSetCheck.process`: implements `FileSetCheck.process`: sets up environment and parameters, filters target file extension and delegates to subclass for specific checking logic
